...
class encoder_model (nn.Module) :
  ...
  def __init__(self,args,metric_module,elmo,go_def,**kwargs):
    ...
    #first parameter
    x = torch.randn(args.def_emb_dim)
    x.requires_grad=True
    self.A1 = nn.Parameter(x)
    
    #second parameter
    y = torch.randn(args.def_emb_dim)
    y.requires_grad=True
    self.A2 = nn.Parameter(y)
  ...
  def encode_label_desc (self,label_idx,label_len):
   ...
   # label_emb = self.dropout(label_emb)
    ...
    label_emb = self.elmo(word_vecs)['elmo_representations'][0] * self.A1 + self.elmo(word_vecs)['elmo_representations'][1] * self.A2
    encode_sum = torch.sum(label_emb.cuda(), dim = 1).cuda()
    average_sum = encode_sum/label_len 
    return average_sum.cuda() # one vector for each label, so batch x dim
  ...
  def do_train(self,train_dataloader,dev_dataloader=None):
    ...
    # {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay)] , 'weight_decay': 0.01},
      {'params': [p for n, p in param_optimizer] + [p for n, p in list(self.metric_module.named_parameters())] + [self.A2] + [self.A1] , 'weight_decay': 0.0}
      ]
    #params = [p for n,p in self.named_parameters () if "word_embedding" not in n]
    ...
    for epoch in range( int(self.args.epoch)) :
      ...
      for step, batch in enumerate(tqdm(train_dataloader, desc="ent. epoch {}".format(epoch))):
        ...
        label_name1,_ , _ , _, label_name2, _, _, _, label_ids = batch
